...
function MemoPane({ entry }: { entry: Entry }) {
  ...
  const handleEditStart = (note: MemoNote) => {
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2 min-h-0">
        {loading ? (
          ...
          <div key={note.id} className="group flex items-end justify-end gap-1">
            {/* 編集・削除ボタン */}
            <div className="opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-1 shrink-0 mb-1">
              <button
              ...
              {editingId === note.id ? (
                <div className="flex flex-col gap-1">
                  <textarea
                    ...
                    className="w-full text-sm px-3 py-2 bg-white border-2 border-indigo-400 rounded-2xl rounded-br-sm outline-none focus:ring-2 focus:ring-indigo-300 resize-none"
                  />
